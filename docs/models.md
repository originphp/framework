...
### Create
When you create a record you will usually take data from the request (i.e. when sombody submits a form) or if you need to create something on the fly. 
```php
...
```
The list finder will return an arrays like `['a','b','c']` or `['a'=>'b']` or `['c'=>['a'=>'b']]`
```php
...
Hint: If you are using a good ide like [Visual Studio Code](https://code.visualstudio.com/), when you type in the function name, it will autocomplete it for you with the arguments.
...
class Product extends AppModel
{
    public function initialize(array $config){
        ...
        $this->validate('username', [    // single rule
         ...
         $this->validate('email', [ // multiple rules
            'rule1' =>  ['rule'=>'notBlank'],
            'rule2' =>  ['rule'=>'email']
        ]);
    }
